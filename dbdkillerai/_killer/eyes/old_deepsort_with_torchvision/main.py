...
print(f"Detector: {args.yolo}")
print(f"Re-ID embedder: {args.embedder}")

# Load model.
if args.yolo:
    model = YOLO(args.yolo + '.pt')
    # model = torch.hub.load('ultralytics/yolov5', args.yolo, pretrained=True)
else:
    ...
    if ret:
        if args.imgsz != None:
            resized_frame = cv2.resize(
                cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), (args.imgsz, args.imgsz)
            )
            # resized_frame = np.rollaxis(resized_frame, -1, 0)
        else:
        ...
        # resized_frame = resized_frame.reshape(1, -1, -1, -1)
        ...
        with torch.no_grad():
            ...
            detections = model(frame_tensor.unsqueeze(0))[0]
